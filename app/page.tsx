...
function Slot({ s, variant }: { s: any; variant: string }) {
  return (
    <div className="flex items-center border-b border-warm-border bg-white">
      <div className="w-16 shrink-0 py-3 text-center text-xs text-warm-sub border-r border-warm-border">{s.time}</div>
      <div className="flex-1 flex items-center px-3 py-3 gap-2.5">
        <span className={`text-[10px] px-2 py-0.5 rounded font-medium shrink-0 ${s.color} ${s.bg} border ${s.border}`}>{s.tag}</span>
        <span className="text-sm text-warm-text leading-snug">{s.name}</span>
        <span className="ml-auto text-[10px] text-warm-dim shrink-0">{s.dur}</span>
      </div>
    </div>
  )
}
    ...
    <main className="min-h-screen bg-cream">
      ...
      <footer className="bg-white border-t border-warm-border py-6 text-center text-xs text-warm-dim">
        <div className="flex items-center justify-center gap-4 mb-2">
          ...
          <img src="/p404-logo.png" alt="PROJECT 404" className="h-4 opacity-50" />
        </div>
